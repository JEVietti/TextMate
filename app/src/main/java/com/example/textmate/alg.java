...
public class alg{
    ...
    //Class Variables
    private int divisor; //Used to keep track of divisor and multiplier in order to find the averages
   ...
   private double valNum,valSize,valTime,valueYesterday,valueToday,valueAvg; //Store Value of Yesterday and Today to compare
                                                                       //idea to store the average value
    ...
    //Constructors
    public alg(int numTxt,int txtNumWord,double txtTimeSent,double txtTimeReceived,double valYesterday,double valAvg){
        this.wordCount = numTxt;
        this.msgCount = txtNumWord;
        this.avgTimeRec = txtTimeReceived;
        this.avgTimeSent = txtTimeSent;
        this.valueYesterday = valYesterday;
        this.valueAvg = valAvg;
        ...
        this.relStatus = printStatusRel(this.valueToday,this.valueYesterday);
    }
...
/* For the function valNumTxt -> the value assigned based on the # of texts
  ...
  * The weight of which will be (#texts / time) */
    ...
    double valSizeTxt(int x,int y){return (x+y)/24;}
    ...
    * */
    ...
    double valTimeTxt(double tS,double tR){
        double ans,tSR;
        tSR = Math.abs(tS-tR);
